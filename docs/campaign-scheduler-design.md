...
> 状态: LIVE relay implemented · 2026-07-30 · Owner: Marcus
...
LIVE 值班采用“完成后再接力”，不是预先堆 Queue：当前 Campaign 完整结束并把
结果写入 SQLite 后，Scheduler 最多约 30 秒内重新读取到期名单与客户回复，再
自动启动下一批。这样不需要人工按「下一批」，也不会在上一批状态尚未推进时重复
选中同一位客户。没有工作或安全闸门暂时失败时，每五分钟重新检查一次。
...
- **总开关**: `关闭` 直接停。
- LIVE 需明确 arm("我知道这会发真实客户"),默认关。旧版曾经选择过但当时被
  锁住的 LIVE 设定不会自动继承 arm 权限。
...
3. **接真实客户 = LIVE** —— 已完成；明确 arm 后复用 Next Flow 的名单、
   device ownership、STOP、回复检查与模板流程。
4. **值班循环** —— 已完成；10–21 内一批落地后自动重新检查并接下一批。
