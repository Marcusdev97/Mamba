...
- **Control Center**(`:8810`,`campaign-app/control_center.mjs`)—— 把所有 `.command` 启动器列成按钮的总面板。
...
3. **启动 WhatsApp 引擎**——双击 **🐳 启动 Evolution**(会拉起 Docker / Colima + Evolution 容器),然后在主控制台「+ 添加号码(扫码)」扫码上线。

4. **开面板**——双击 **Mamba Control Center**,所有按钮就出来了。
