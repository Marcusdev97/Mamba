...
assert.match(sendHtml, /id="monitorHideAll"/);
assert.match(sendHtml, /id="monitorCancelAll"/);
assert.match(sendScript, /\/api\/campaign\/runs\/cancel-all/);
assert.match(sendScript, /confirmation: "CANCEL_MONITOR_RUNS"/);
...
assert.match(sendHtml, /id="campaignRunDetailClose"[^>]*>关闭详情/);
...
assert.match(sendScript, /syncCampaignRunDetail\(visible\)/, "Hidden Campaign cards must not remain open in the Flow detail panel");
assert.match(sendScript, /frame\.src = "about:blank"/, "Closing the detail must stop its hidden iframe polling");
