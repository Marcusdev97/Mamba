...
// Recall 的窗口。取「最近 15 条」和「最近 14 天」先到的那个 —— 再多就会把
...
async function sendWhatsApp(instanceName, number, text) {
  ...
  await logOutbound(instanceName, number, text, result);
  return result;
}

// 大脑回出去的话也要进本机数据库，不然 conversation 只剩客户那一半。
// service 自己会挡掉名单外的号码。写库失败绝不能影响回复 —— 讯息已经发出去了。
async function logOutbound(instanceName, number, text, result) {
  try {
    ...
    console.log(`[brain] 对话纪录写入失败(讯息已发出) ${number}: ${error.message}`);
  }
}
...
async function handleEvent(event) {
  ...
  try {
    if (aiEnabled) {
      ...
      // Recall：把这个客户之前的对话拉出来当记忆。读不到就当没有 —— 少了记忆的
      // 草稿还是能用，为了记忆卡住整个回复反而更糟。
      const history = await conversationLog
        .recentThread(event.phone, { limit: HISTORY_LIMIT, sinceDays: HISTORY_DAYS, excludeId: event.id })
        .catch((error) => {
          console.log(`[brain] 读不到对话历史 ${event.phone}: ${error.message}`);
          return [];
        });
      if (history.length) console.log(`[brain] recall ${history.length} 条历史 -> ${event.phone}`);
      const prompt = buildPrompt({ event, classified, cache, lead, projectCtx, history });
      if (SIMULATE) {
